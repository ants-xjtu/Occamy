...
static int parse_next_hop_eth_addr(char *cfg_file) {
  FILE *fp;
  char buff[20];
  char *tok;
...
int app_parse_args(int argc, char **argv) {
  ...
  uint32_t max_lcores = APP_CORE_NUM;
  ...
  for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
    ...
    if (n_lcores >= max_lcores) {
      RTE_LOG(ERR, USER1, "Number of cores must be %u\n", max_lcores);
      app_print_usage();
      return -1;
    }
  ...
  app.core_func = lcores[2];
  app.core_debug = lcores[3];
  app.core_threshold = lcores[4];
  app.core_update_aqm = lcores[5];
  uint32_t i;
  for (i = 0; i < APP_MAX_PORTS; i++)
    app.core_tx[i] = lcores[i + 6];
  /* Non-EAL args */
  argvopt = argv;
  while ((opt = getopt_long(argc, argvopt, "p:n:t:a:f:m:", lgopts,
                            &option_index)) != EOF) {
    switch (opt) {
    case 'p':
    ...
    case 't':
      app.method = atoi(optarg);
      break;

    /* dt shift alpha */
    case 'a':
      app.dt_shift_alpha = atoi(optarg);
      break;

    case 'm':
      strcpy(app.mac_file, optarg);
      break;
