    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    RegisterAction<q_len_t, q_index_t, bit<3>>(thres_reg) get_drop = {
        void apply(inout q_len_t thres, out bit<3> drop) {
            if (thres < meta.length + 20 || thres > MAX_LEN * ALPHA__2) {
                drop = 1;
            } else {
                drop = 0;
            }
    ...
    apply {
        if (hdr.sync.isValid()) {       // 是同步包
            ...
            ig_tm_md.bypass_egress = (bit)ig_dprsr_md.drop_ctl;
        }
    }
}
    ...
    inout egress_intrinsic_metadata_for_output_port_t  eg_oport_md)
{
    ...
    apply {
        if (hdr.sync.isValid()) {
            ...
            if (meta.queued == true) {
#if HEAD_DROP
                eg_dprsr_md.drop_ctl = get_drop.execute(0);
#endif
            }
        }
    }
