    ...
    for requestSizeRate in ${requestSizeRate_array[*]};do        
        for tcpProtocol in ${tcpProtocol_array[*]};do
            for method in ${method_array[*]};do
                ...
                while [[ $(ps aux|grep "sig_100g_allreduce-optimized"|wc -l) -gt $N_CORES ]];do
                    sleep 10;
                    echo "waiting for cores, $N running..."
                done
                N=$(( $N+1 ))
                echo "./ns3 run \"scratch/sig_100g_allreduce.cc --method=${method} --alpha=${alpha} --tcpProtocol=${tcpProtocol} --webLoad=${webLoad} --requestSizeRate=${requestSizeRate} --requestFlowRate=${requestFlowRate} --bufferSize=${bufferSize} --nPrior=${nPrior}\""
                ./ns3 run "scratch/sig_100g_allreduce.cc --method=${method} --alpha=${alpha} --tcpProtocol=${tcpProtocol} --webLoad=${webLoad} --requestSizeRate=${requestSizeRate} --requestFlowRate=${requestFlowRate} --bufferSize=${bufferSize} --nPrior=${nPrior}" > /dev/null &
                sleep 2
                echo $N
            done
        done
